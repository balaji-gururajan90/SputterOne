...
function formatTorrScientific(value) {
    ...
    return `${mantissa.toFixed(1)} x 10^${exponent} Torr`;
}

function getDischargeStabilityWarning() {
    if (STATE.pressure >= 0.5) return '';
    return 'Warning: Below typical magnetron discharge stability limit (~0.5 mTorr). Real discharges may extinguish at this pressure.';
}
...
function updateGrowthSurfaceState(matKey) {
    const activeMat = matKey || STATE.sources[0]?.material || STATE.material || 'Cu';
    STATE.deltaGamma = computeDeltaGamma(activeMat, STATE.substrate);
}

function updateGrowthDiagnosticsDisplay() {
    ...
    if (gmEl) {
        ...
        if (resolved === 'auto') {
            resolved = classifyGrowthModeFromSurfaceState(STATE.deltaGamma, STATE.latticeMismatch, STATE.surfaceMobility);
        }
    ...
    const dgEl = document.getElementById('stat-delta-gamma');
    if (dgEl) {
        dgEl.innerText = STATE.deltaGamma.toFixed(2);
        dgEl.style.color = STATE.deltaGamma > 0 ? '#e8903a' : 'var(--accent-cyan)';
    }

    const subEl = document.getElementById('stat-substrate');
    if (subEl) {
        const sub = SUBSTRATE_DB[STATE.substrate] || SUBSTRATE_DB.Si;
        subEl.innerText = sub.label;
    }
}
...
function getUniformityMetrics() {
    ...
    let thicknessData = rawThickness.map(v => Number.isFinite(v) ? v : 0);
...
function calculateYamamuraYield(ionEnergyEV, matProps, thetaRad = 0) {
    const Eth = Math.max(8, matProps.Eth || (matProps.U0 || matProps.Eb || 3) * 6.5);
    if (ionEnergyEV <= Eth) return 0;

    const z1 = PROJECTILE_AR.Z;
    const z2 = Math.max(1, matProps.Z || 29);
    const M1 = PROJECTILE_AR.mass;
    const M2 = Math.max(1, matProps.mass || 63.5);
    const gamma = (4 * M1 * M2) / Math.pow(M1 + M2, 2);
    const aL = 0.4685 / Math.sqrt(Math.pow(z1, 2 / 3) + Math.pow(z2, 2 / 3));
    const eps = Math.max(1e-6, (aL * M2 * Math.max(ionEnergyEV, 0.1)) / (z1 * z2 * 14.4 * (M1 + M2)));
    const sqrtEps = Math.sqrt(eps);
    const sn = (3.441 * sqrtEps * Math.log(eps + Math.E)) /
        (1 + 6.355 * sqrtEps + eps * (6.882 * sqrtEps - 1.708));
    const q = matProps.yamQ || 0.1;
    const lowEnergyTerm = Math.max(0, 1 - Math.pow(Eth / ionEnergyEV, 2 / 3));
    const thresholdTail = Math.max(0, 1 - Eth / ionEnergyEV);
    const normalIncidenceYield = q * sn * Math.pow(lowEnergyTerm, 2) * Math.pow(thresholdTail, 2);
...
//        const contribution = depositedAmount * fraction * yieldMult;
        ...
        const _depScaleMap = {
            ...
            'Zn': 0.40, 'Sn': 0.85, 'Ag': 1.00, 'W': 0.70,
            'Ta': 0.65, 'Mo': 0.80
        };
...
function computeInterfacialEnergy(gamma_film, gamma_sub) {
    // Geometric mean approximation from standard wetting theory.
    return gamma_film + gamma_sub - 2.0 * Math.sqrt(gamma_film * gamma_sub);
}

function computeDeltaGamma(matKey, subKey) {
    const mat = MATERIALS[matKey] || MATERIALS.Cu;
    const sub = SUBSTRATE_DB[subKey] || SUBSTRATE_DB.Si;
    const gamma_film = mat.gamma_film || 1.79;
    const gamma_sub = sub.gamma;
    const gamma_int = computeInterfacialEnergy(gamma_film, gamma_sub);
    return gamma_film + gamma_int - gamma_sub;
}

function classifyGrowthModeFromSurfaceState(deltaGamma, mismatch, mobility) {
    if (mismatch > 0.07) return 'SK';
    if (deltaGamma <= 0 && mobility > 0.4) return 'FM';
    if (deltaGamma > 0.5) return 'VW';
    return 'SK';
}
...
function recommendGrowthModeForRecipe(recipe, metrics) {
    ...
    const deltaGamma = computeDeltaGamma(recipe.material || STATE.sources[0]?.material || STATE.material || 'Cu', STATE.substrate);
    const mode = classifyGrowthModeFromSurfaceState(deltaGamma, mismatch, mobilityIndex);
    if (mode === 'SK' && mismatch > 0.07) return { code: 'SK', label: 'Stranski-Krastanov', reason: 'higher mismatch favors an initial wetting layer followed by islands' };
    if (mode === 'FM') return { code: 'FM', label: 'Frank-van der Merwe', reason: 'negative surface free energy balance and sufficient mobility favor layer-by-layer growth' };
    if (mode === 'VW') return { code: 'VW', label: 'Volmer-Weber', reason: 'positive surface free energy balance favors island-like nucleation' };
    return { code: 'SK', label: 'Stranski-Krastanov', reason: 'intermediate wetting balance favors a wetting layer followed by islands' };
}
...
const MATERIALS = {
    'Cu': { name: 'Copper', Eb: 3.49, U0: 3.49, gamma_film: 1.79, color: [184, 115, 51], sputterTint: [212, 108, 72], mass: 63.5, Z: 29, yamQ: 0.115, angleShape: 1.55, Eth: 24 },
    'Al': { name: 'Aluminum', Eb: 3.39, U0: 3.39, gamma_film: 1.16, color: [210, 210, 220], sputterTint: [220, 196, 92], mass: 27.0, Z: 13, yamQ: 0.150, angleShape: 1.48, Eth: 21 },
    'Ti': { name: 'Titanium', Eb: 4.89, U0: 4.89, gamma_film: 2.57, color: [135, 134, 139], mass: 47.9, Z: 22, yamQ: 0.092, angleShape: 1.62, Eth: 28 },
    'Ta': { name: 'Tantalum', Eb: 8.10, U0: 8.10, gamma_film: 3.15, color: [138, 141, 143], mass: 180.9, Z: 73, yamQ: 0.050, angleShape: 1.82, Eth: 36 },
    'Zn': { name: 'Zinc', Eb: 1.35, U0: 1.35, gamma_film: 0.99, color: [173, 210, 180], sputterTint: [224, 138, 64], mass: 65.4, Z: 30, yamQ: 0.208, angleShape: 1.42, Eth: 14 },
    'Sn': { name: 'Tin', Eb: 3.12, U0: 3.12, gamma_film: 0.68, color: [200, 200, 212], mass: 118.7, Z: 50, yamQ: 0.112, angleShape: 1.50, Eth: 20 },
    'Ag': { name: 'Silver', Eb: 2.97, U0: 2.97, gamma_film: 1.25, color: [192, 192, 192], sputterTint: [192, 192, 192], mass: 107.9, Z: 47, yamQ: 0.508, angleShape: 1.50, Eth: 15, density: 10.49 },
    'W': { name: 'Tungsten', Eb: 8.83, U0: 8.83, gamma_film: 4.01, color: [120, 120, 140], mass: 183.8, Z: 74, yamQ: 0.046, angleShape: 1.86, Eth: 38 },
    'Mo': { name: 'Molybdenum', Eb: 6.83, U0: 6.83, gamma_film: 3.00, color: [100, 110, 130], mass: 95.9, Z: 42, yamQ: 0.074, angleShape: 1.72, Eth: 31 }
};
...
const SUBSTRATE_DB = {
    'Si': {
        ...
        visual: {
            ...
            face: [38, 66, 92],
            stroke: [85, 190, 235],
            highlight: [158, 226, 248],
            text: [184, 224, 242],
            shadow: 'rgba(45,175,255,0.22)'
        }
    },
    'Glass': {
        name: 'Glass SiO2',
        ...
        visual: {
            ...
            face: [110, 194, 212],
            stroke: [158, 236, 255],
            highlight: [232, 255, 255],
            text: [216, 248, 255],
            shadow: 'rgba(120,220,255,0.24)'
        }
    },
    'Sapphire': {
        name: 'Sapphire Al2O3',
        ...
        visual: {
            ...
            face: [74, 108, 188],
            stroke: [142, 184, 255],
            highlight: [204, 224, 255],
            text: [208, 224, 255],
            shadow: 'rgba(105,150,255,0.25)'
        }
    },
};
// gamma values in J/m2 from Ohring (2002), Materials Science of Thin Films.

function getSubstrateVisualStyle() {
    const substrate = SUBSTRATE_DB[STATE.substrate] || SUBSTRATE_DB.Si;
    return substrate.visual || SUBSTRATE_DB.Si.visual;
}

const PROJECTILE_AR = { symbol: 'Ar', mass: 39.95, Z: 18 };

function ensureMaterialOptions() {
    const selectIds = ['materialSelect', 'aiMaterial', 'src1MatSelect', 'src2MatSelect', 'src3MatSelect'];
    selectIds.forEach(id => {
        const select = document.getElementById(id);
        if (!select) return;
        Object.entries(MATERIALS).forEach(([key, mat]) => {
            if (Array.from(select.options).some(option => option.value === key)) return;
            const option = document.createElement('option');
            option.value = key;
            option.textContent = `${mat.name} (${key})`;
...
function setup() {
    try {
        ...
        bindUIEvents();
        updateGrowthSurfaceState();
        updateGrowthDiagnosticsDisplay();
...
function bindUIEvents() {
    ...
    document.getElementById('materialSelect').addEventListener('change', (e) => {
        ...
        STATE.deltaGamma = computeDeltaGamma(e.target.value, STATE.substrate);
        updateStaticYield();
        updateGrowthDiagnosticsDisplay();
    });
    ...
    if (substrateSelect) {
        substrateSelect.addEventListener('change', (e) => {
            STATE.substrate = e.target.value;
            const matKey = STATE.sources[0].material || 'Cu';
            STATE.deltaGamma = computeDeltaGamma(matKey, STATE.substrate);
            updateGrowthDiagnosticsDisplay();
            refreshProcessDiagnosticsFromState();
        });
    }

    const hqPlasmaCheck = document.getElementById('hqPlasmaCheck');
    if (hqPlasmaCheck) hqPlasmaCheck.addEventListener('change', (e) => {
    ...
    const drawMagLinesCheck = document.getElementById('drawMagLinesCheck');
    if (drawMagLinesCheck) drawMagLinesCheck.addEventListener('change', (e) => {
    ...
    const drawElectronsCheck = document.getElementById('drawElectronsCheck');
    if (drawElectronsCheck) drawElectronsCheck.addEventListener('change', (e) => {
    ...
    const substrateRotCheck = document.getElementById('substrateRotCheck');
    if (substrateRotCheck) substrateRotCheck.addEventListener('change', (e) => {
    ...
    [
        ['hqPlasmaCheck', 'hqPlasma'],
        ['drawMagLinesCheck', 'drawMagLines'],
        ['drawElectronsCheck', 'drawElectrons'],
        ['substrateRotCheck', 'substrateRotEnabled']
    ].forEach(([id, stateKey]) => {
        const el = document.getElementById(id);
        if (!el) return;
        STATE[stateKey] = el.checked;
        if (el.checked && el.parentElement) el.parentElement.classList.add('checked');
    });
    updateControlRelevance();
    ...
    document.querySelectorAll('[data-geometry-tab]').forEach(btn => {
        btn.addEventListener('click', () => {
            const tab = btn.getAttribute('data-geometry-tab');
            document.querySelectorAll('[data-geometry-tab]').forEach(item => {
                const isActive = item === btn;
                item.classList.toggle('active', isActive);
                item.setAttribute('aria-selected', isActive ? 'true' : 'false');
            });
            document.querySelectorAll('[data-geometry-panel]').forEach(panel => {
                panel.classList.toggle('active', panel.getAttribute('data-geometry-panel') === tab);
            });
        });
    });
...
function drawSubstrateHolder(subWidthPx) {
    const holderH = 14;
    const style = getSubstrateVisualStyle();
    const substrate = SUBSTRATE_DB[STATE.substrate] || SUBSTRATE_DB.Si;
    push();
    rectMode(CENTER);
    noStroke();
    ...
    drawingContext.shadowColor = style.shadow;
    fill(style.base[0], style.base[1], style.base[2], 244);
    rect(0, 0, subWidthPx, holderH, 3);
    drawingContext.shadowBlur = 0;

    stroke(style.stroke[0], style.stroke[1], style.stroke[2], 206);
    strokeWeight(1.1);
    fill(style.face[0], style.face[1], style.face[2], STATE.substrate === 'Glass' ? 146 : 188);
    rect(0, 0, subWidthPx - 2, holderH - 2, 3);

    noStroke();
    fill(style.highlight[0], style.highlight[1], style.highlight[2], STATE.substrate === 'Glass' ? 72 : 42);
    rect(0, -holderH * 0.16, subWidthPx * 0.94, holderH * 0.28, 2);

    if (!STATE.isRunning && getVisibleFilmThicknessNm() <= 0.05) {
        textAlign(CENTER, CENTER);
        textSize(7.5);
        textFont('Cascadia Mono, Consolas, monospace');
        fill(style.text[0], style.text[1], style.text[2], 138);
        text(substrate.label.toUpperCase(), 0, holderH * 0.88);
    }
    ...
    if (STATE.substrateRotEnabled) {
        ...
        drawingContext.clip();
        stroke(style.highlight[0], style.highlight[1], style.highlight[2], 116);
        strokeWeight(0.8);
        ...
        noStroke();
        fill(style.stroke[0], style.stroke[1], style.stroke[2], 148);
...
function updateStaticYield() {
    ...
    const _yieldScaleMap = {
        'Cu': 161, 'Al': 26.3, 'Ti': 53.9,
        'Zn': 136, 'Sn': 142, 'Ag': 56, 'W': 108,
        'Ta': 84, 'Mo': 87
    };
...
function resetLabDefaults() {
    ...
    const substrateSelect = document.getElementById('substrateSelect');
    if (substrateSelect) substrateSelect.value = 'Si';
    ...
    if (substrateSelect) substrateSelect.dispatchEvent(new Event('change'));
...
class SputteredAtom {
    ...
    draw() {
        let partOp = (typeof STATE !== 'undefined' && STATE.opPart !== undefined) ? STATE.opPart : 1.0;
        noStroke();
...
function handleDeposition(x, sourceIdx = 0, depositionWeight = 1.0) {
    ...
    if (mode === 'auto') {
        const dg = STATE.deltaGamma;
        const mis = STATE.latticeMismatch;
        const mob = STATE.surfaceMobility;
        mode = classifyGrowthModeFromSurfaceState(dg, mis, mob);
    }
...
function draw() {
    try {
        ...
        rectMode(CENTER);
        const substrateStyle = getSubstrateVisualStyle();
        drawingContext.shadowBlur = 0;
        stroke(substrateStyle.stroke[0], substrateStyle.stroke[1], substrateStyle.stroke[2], 118);
        strokeWeight(0.9);
        fill(substrateStyle.base[0], substrateStyle.base[1], substrateStyle.base[2], 210);
...
function buildSummaryCSVRows() {
    ...
    STATE.sources.forEach((src, idx) => {
        rows.push([
            ...
            src.powerType,
            (src.yield * ({
                'Cu': 161, 'Al': 26.3, 'Ti': 53.9, 'Zn': 136,
                'Sn': 142, 'Ag': 56, 'W': 108, 'Ta': 84, 'Mo': 87
